...
function createArray() {
    arr = [];
    $("#array").html(''); // Clear the existing array

    for (var i = 0; i < size; i++) {
        var n = Math.floor(Math.random() * (MAX - MIN + 1)) + MIN;
        arr.push(n);

        var $element = $('<div>'); // Create the bar element
        $element.attr('id', "e" + i);
        $element.attr('class', "element");
        $element.css('background-color', UNSORTED);
        $element.css('width', element_width.toString() + 'px');
        ...
        // Add a span element to display the number inside the bar
        var $number = $('<span>');
        $number.attr('class', 'bar-number');
        $number.text(n); // Set the number as the text inside the span
        $number.css('color', 'white'); // Set the text color to be visible
        $number.css('position', 'absolute');
        $number.css('bottom', '5px'); // Position the text near the bottom of the bar
        $number.css('left', '50%');
        $number.css('transform', 'translateX(-50%)');
        $number.css('font-size', '12px'); // Adjust the font size as per design

        $element.append($number); // Append the number to the bar element

        $element.appendTo("#array"); // Add the bar element to the container
    }
}
    ...
    $("#sort").click(
        async function() {
            ...
            if(algo_selected == "Bubble Sort")
                ...
                await heapsort();
            else if (algo_selected == "Radix Sort")
                await radixSort(); 
            else if (algo_selected == "Shell Sort")
                await shellSort();
            else if (algo_selected == "Counting Sort")
                await countingSort();
            else if (algo_selected == "Bucket Sort")
                await bucketSort();
